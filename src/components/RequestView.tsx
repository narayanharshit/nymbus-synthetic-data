    ...
    <div className="thin-scroll min-h-0 flex-1 overflow-y-auto bg-surface">
      <div className="flex items-center justify-between border-b border-line px-4 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="micro">Test data request</span>
          {source && (
      ...
      <div className="max-w-2xl px-4 py-4">
        <p className="mb-4 text-[12.5px] text-ink-muted">
          This is the structured request the dataset will be built from. Review and edit it on the
          left, then generate — or export the request on its own to hand off or save.
        </p>

        <dl className="grid grid-cols-[150px_1fr] gap-x-4 gap-y-0 text-[13px]">
          <Row label="Institution" value={spec.institutionType === "credit_union" ? "Credit union" : "Community bank"} />
          <Row
            label="Products"
            value={spec.products.map((p) => PRODUCT_LABELS[p]).join(", ")}
          />
          ...
          <Row
            label="Edge cases"
            value={enabledEdges.length ? enabledEdges.map((k) => EDGE_LABELS[k]).join(", ") : "none"}
          />
        ...
        <p
          className={cn(
            "mt-5 text-[12px]",
            generating ? "text-accent" : confidence === "low" ? "text-fail" : "text-ink-faint",
          )}
        >
          {generating
            ? "Generating the full dataset…"
            : confidence === "low"
              ? "Low confidence in this reading — review every field before generating."
              : "Edit on the left, then press Generate to produce the dataset."}
        </p>
      </div>
    </div>
  );
